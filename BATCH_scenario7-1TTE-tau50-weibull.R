...
## setwd(path)
## source("BATCH_scenario7-1TTE-tau50-weibull.R")
...
## * path
path <- "."
path.res <- file.path(path,"Results","scenario7-1TTE-tau50-weibull")
if(dir.exists(path.res)==FALSE){
    dir.create(path.res)
}
path.output <- file.path(path,"output","scenario7-1TTE-tau50-weibull")
...
grid <- expand.grid(n = c(200), ## sample size (each group)
...
simData <- function(n.C, n.T, hazard.C, hazard.T, hazard.censoring, shape){

    group <- c(rep("T", n.T), rep("C", n.C)) # on attribut un groupe a chacun des patients
    TimeToEvent <- c(rweibull(n.T, shape = shape, scale=1/hazard.T), rweibull(n.C, shape = shape, scale=1/hazard.C)) # CJ1 ex:OS
...
for (iGrid in 1:n.grid){   ## iGrid <- 1
    ...
    iShape <- grid[iGrid,"shape"]
    
    cat("Grid (",iGrid,"/",n.grid,") : n=",iN," | hazard.T=",iHazard.T," | hazard.C=",iHazard.C," | hazard.censoring=",iHazard.censoring," | shape=",iShape,"\n", sep = "")
